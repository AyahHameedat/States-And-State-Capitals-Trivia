...
//        Stream<StateInfo> list = firstFiveStates.stream();
//        list.limit(5).collect(Collectors.toList());
        ...
        long count = states.stream().count();
        List<StateInfo> lastFiveStates = states.stream().skip(count - 5).collect(toList());
...
//        collect(toList());
////        long count1 = firstFiveNumbers.stream().count();
//        firstFiveNumbers.stream().limit(5).collect(toList());
...
//        StateInfo cardinalState = states.stream().findAny();
//        getRoles().stream()
//                .filter(role -> role.getId().equals(id))
//                .findAny();
//        Optional<Integer> first = list.stream().findFirst();
        StateInfo cardinalState = states.stream().filter(stateInfo -> stateInfo.getStateBird().equals("cardinal")).collect(toList()).get(0);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(stateInfo -> stateInfo.getStateAnthem().equals("anthem"));
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(stateInfo -> stateInfo.getStateMotto().length() != 1);
...
//                    num -> Double.parseDouble(num)));
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(Collectors.averagingDouble(StateInfo -> StateInfo.getCapital().getAverageYearlyPrecipitationInInches()));
...
//                .sum();
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(StateInfo -> StateInfo.getCapital().getAverageYearlyPrecipitationInInches()).sum();
...
//                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(StateInfo -> StateInfo.getTimeZones().toString(), Collectors.counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(Collectors.groupingBy(StateInfo -> StateInfo.getCapital().getTimeZone(), Collectors.counting()));;
...
//                .collect(Collectors.joining(", "));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo -> StateInfo.getStateBird()).distinct().filter(StateInfo -> !(StateInfo.contains("mockingbird"))).collect(toList());
        ...
        Long numberOfDistinctStateBirds = states.stream().map(StateInfo -> StateInfo.getStateBird()).distinct().collect(counting());
...
//        states.stream().max(Comparator.comparing(StateInfo::getStateName)
        Integer maxStateElevation = states.stream().max(Comparator.comparing(StateInfo::getHighestElevationInFeet)).get().getHighestElevationInFeet();
